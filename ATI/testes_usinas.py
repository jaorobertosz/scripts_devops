...
def adicionar_backup(cliente):
    try:
        ...
        novo_diretorio = f"/scripts_devops/USINAS/{cliente}/BACKUP/"
        ...
        shutil.copyfile(f"{diretorio}verifica_backup_CLIENTE.sh", f"{novo_diretorio}verifica_backup_{cliente}.sh")
        print(f"Arquivo copiado para: {novo_diretorio}verifica_backup_{cliente}.sh")

        print("Atualizando o arquivo verifica_backup_CLIENTE com o nome do cliente...")
        with open(f"{novo_diretorio}verifica_backup_{cliente}.sh", 'r+') as f:
            conteudo = f.read()
            f.seek(0)
