...
BKPJQBS_ETH1='172.31.1.251'
BKPJQBN_ETH2='172.31.1.252'
BKPPDMN_ETH1='172.31.10.251'
BKPPDMS_ETH2='172.31.10.252'
USERZBX='suporteati'
...
#	BACKUP ETH1
	...
	sleep 5s
	scp "$BKPPDMN_ETH1:$BKPDIRUTR" "$BKPDIR" && echo "$(date +%d%m%Y%H%M) - $MSG1" && echo "$(date +%d/%m/%Y-%H:%M:%S) - $MSG3" || echo "$(date +%d/%m/%Y-%H:%M:%S) - $MSG4" && chown "$USERZBX". "$BKPDIR" -R && echo "$DATA - $MSG5"	
...
#	BACKUP ETH2
	...
	sleep 5s
	scp "$BKPPDMS_ETH2:$BKPDIRUTR" "$BKPDIR" && echo "$(date +%d%m%Y%H%M) - $MSG1" && echo "$(date +%d/%m/%Y-%H:%M:%S) - $MSG3" || echo "$(date +%d/%m/%Y-%H:%M:%S) - $MSG4" && chown "$USERZBX". "$BKPDIR" -R && echo "$DATA - $MSG5" 	
