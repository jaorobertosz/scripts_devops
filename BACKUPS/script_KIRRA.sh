...
CLIENTE='KIRRA'
USINA='KIRRA'
BKPETH1='172.33.1.251'
BKPETH2='172.33.1.252'
USERZBX='suporteati'
BKPDIRUTR='/root/BACKUP/BKP_*'
BKPDIR='/USINAS/'$CLIENTE'/'$USINA'/BACKUP/'
...
#	BACKUP MATHEUS-LEME_ETH2
	scp "$BKPETH2:$BKPDIRUTR" "$BKPDIR" && echo "$(date +%d%m%Y%H%M) - $MSG1" && echo "$(date +%d/%m/%Y-%H:%M:%S) - $MSG3" || echo "$(date +%d/%m/%Y-%H:%M:%S) - $MSG4" && chown "$USERZBX". "$BKPDIR" -R && echo "$DATA - $MSG5" 
